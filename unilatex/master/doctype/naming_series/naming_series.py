...
class NamingSeries(Document):
	def get_transactions(self, arg=None):
		doctypes = list(set(frappe.db.sql_list("""select parent from `tabDocField` df where fieldname='naming_series'""") + frappe.db.sql_list("""select dt from `tabCustom Field` where fieldname='naming_series'""")))
	...
	def scrub_options_list(self, ol):
		#options = list(filter(lambda x: x, [cstr(n).strip() for n in ol])) # add list to python3
	...
	def set_series_for(self, doctype, ol):
		...
		for prop in prop_dict:
			ps_exists = frappe.db.get_value("Property Setter", {"field_name": 'naming_series', 'doc_type': doctype, 'property': prop})
	...
	def update_series_start(self):
		if self.prefix:
			...
			frappe.db.sql("update `tabSeries` set current = %s where name = %s", (self.current_value, prefix))
...
def set_by_naming_series(doctype, fieldname, naming_series, hide_name_field=True):
	...
	if naming_series:
		...
		if hide_name_field:
			...
			# set values for mandatory
			frappe.db.sql("""update `tab{doctype}` set `{fieldname}`=`name` where ifnull({fieldname}, '')=''""".format(doctype=doctype, fieldname=fieldname))
...
def get_default_naming_series(doctype):
	...
	if not out:
		frappe.throw(_("Please set Naming Series for {0} via Setup > Settings > Naming Series").format(doctype), NamingSeriesNotSetError)
	else:
		return out
